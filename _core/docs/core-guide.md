...
| 폴더 | 내용 | 관리 주체 |
|---|---|---|
| `_core/rules/` | 운영 규칙, 에이전트 행동 규칙, 마크다운 스타일 | 사용자 |
| `_core/commands/` | 커스텀 명령어 정의 | 사용자 |
| `_core/docs/` | 운영 가이드, 설계 이력 | 사용자 + 에이전트 |
| `_core/sessions/` | 세션 핸드오프 문서 | 에이전트 |
| `_core/scripts/` | 자동화 스크립트 | 사용자 |
| `_core/refs/` | 참조 전용 문서 (수정 금지) | — |
| `shared/` | 템플릿에 배포할 공통 파일 원본 | 사용자 |
| `templates/` | 복사 가능한 프로젝트 템플릿 루트 | 사용자 + 에이전트 |
