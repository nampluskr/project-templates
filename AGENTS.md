...
이 파일은 Codex가 이 워크스페이스를 다룰 때 따르는 진입점 지침이다.
상세 규칙은 `_core/` 를 참조한다.
...
- 사용자에게는 한국어 경어체로 응답한다.
- 변경한 파일과 결과를 간단히 요약한다.
- 영향이 큰 작업(템플릿 구조 변경, 파일 삭제)은 실행 전에 의도를 먼저 설명한다.
- 레포 자체 운영 파일(`_core/`)과 템플릿 파일(`templates/`, `shared/`)을 혼동하지 않는다.
